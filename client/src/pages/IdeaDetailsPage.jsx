...
export const IdeaDetailsPage = () => {
  ...
  useEffect(() => {
    const fetchIdea = async () => {
      ...
      try {
        setIsLoading(true);
        const fetchedIdea = await axios.get(
          `http://localhost:4000/api/v1/ideas/${id}`,
          {
            withCredentials: true,
          }
        );
        console.log("Fetched Idea:", fetchedIdea.data.data);
        setIdea(fetchedIdea.data.data);
      } catch (err) {
  ...
  const isCreator = user?._id === idea.submitter?._id;

  // Check if the user is an investor
  const isInvestor = user?.role === 1;
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between">
              <div>
                <div className="flex items-center mb-2">
                  ...
                  {/* {idea.status && (
                    <span
                      className={`ml-3 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusBadgeColor(
                        idea.status
                      )}`}
                    >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2">
                <div className="mb-8">
                  ...
                  <p className="text-gray-700">{idea.businessDescription}</p>
                </div>
